# -*- coding: utf-8 -*-
"""opiate_functions.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1Xmtq-Nn5S3GZKg_TGTqvzn_zhQQrqGZ9
"""
...
def ode_model(z, t, alpha, epsilon, beta_p, beta_a, gamma, zeta, delta, sigma, mu, mu_s):
    ...
    #odeint does some weird approximations with t, so we index parameters by min(int(t), t_max - 1) just to ensure that they are integer values from 0 to t_max
    
    dSdt = -alpha[min(int(t), t_max - 1)]*S - beta_a[min(int(t), t_max - 1)]*S*A - beta_p[min(int(t), t_max - 1)]*S*P \
           + epsilon[min(int(t), t_max - 1)]*P + delta[min(int(t), t_max - 1)]*R + mu[min(int(t), t_max - 1)]*(P + R) \
    ...
    dRdt = zeta[min(int(t), t_max - 1)]*A - (delta[min(int(t), t_max - 1)] + sigma[min(int(t), t_max - 1)] + mu[min(int(t), t_max - 1)])*R
...
#Parameter values taken from https://link.springer.com/article/10.1007/s11538-019-00605-0 however, divided by 12 to get averages per month
